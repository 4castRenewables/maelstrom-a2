...
class GetPower(object):
    ...
    def __exit__(self, type, value, traceback):
        logger.info("Exiting context for energy measurement!")
        self.end_event.set()
        power_value_dict = self.power_queue.get()
        self.smip.join()

        logger.info("Writing dataset in exit context for energy measurement!")
...
def save_energy_to_file(measured_scope, folder, file_id):
    ...
    filename_csv = folder + f"EnergyFile-NVDA-{file_id}.csv"
    measured_scope.df.to_csv(filename_csv)

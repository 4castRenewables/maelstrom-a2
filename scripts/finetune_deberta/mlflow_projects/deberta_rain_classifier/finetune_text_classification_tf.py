...
def Deep500Callback():
    ...
    return timer.TimerCallback(tmr, gpu=True)
...
class ModelArguments:
    ...
    use_deep500: bool = field(
        default=False,
        metadata={"help": ("Whether or not to use deep500 for benchmarking.")},
    )
...
def main():
    ...
    if "train" in datasets:
        ...
        is_regression = datasets["train"].features[KEY_LABELS].dtype in ["float32", "float64"]
        if is_regression:
            num_labels = 1
        else:
            # A useful fast method:
            # https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.unique
            label_list = datasets["train"].unique(KEY_LABELS)
    ...
    non_label_column_names = [name for name in column_names if name != KEY_LABELS]
    non_label_text_column_names = [name for name in column_names if name not in [KEY_LABELS, KEY_TEXT]]
    if KEY_TEXT in non_label_column_names and "sentence2" in non_label_column_names:
        sentence1_key, sentence2_key = KEY_TEXT, "sentence2"
    elif KEY_TEXT in non_label_column_names:
        sentence1_key, sentence2_key = KEY_TEXT, None
    else:
    ...
    if "validation" in datasets and config.label2id is not None:
        validation_label_list = datasets["validation"].unique(KEY_LABELS)
    ...
    def preprocess_function(examples):
        ...
        # Map labels to IDs
        if config.label2id is not None and KEY_LABELS in examples:
            result["label"] = [(config.label2id[_label] if _label != -1 else -1) for _label in examples[KEY_LABELS]]
        return result

    datasets = datasets.remove_columns(non_label_text_column_names)
    datasets = datasets.map(preprocess_function, batched=True, load_from_cache_file=not data_args.overwrite_cache)
    datasets = datasets.remove_columns([KEY_LABELS, KEY_TEXT])
    ...
    with training_args.strategy.scope():
        ...
        for key in ("train", "validation", "test"):
            ...
            if key in ("train", "validation"):
                # assert KEY_LABELS in datasets[key].features, f"Missing labels from {key} data!"
                pass
            if key == "train":
                shuffle = True
